...
def review(message):                        # сей конструкцией мы получаем текст из телеграмма, в нашем случае URL на автора
    ...
    if (message_to_save_add == "https://joyreactor.cc/tag/"):           # исключение для пустого тега
        bot.send_message(message.chat.id, "Передан не верный URL. URL имеет тип https://joyreactor.cc/tag/ник_автора")
        return 0        
    elif (message_to_save_add.find("reactor.cc/tag/") != -1):        # ищем подстроку в строке и возвращаем индекс первого вхождения. для нас достаточно определить что эта подстрока впринципе есть 
        ...
        if (chek_list_key_json(message_to_save_add, str(message.from_user.id)) == 1):      # проверяет есть ли поданый телегой автор в СПИСКЕ авторов json
            bot.send_message(message.chat.id, "такой автор уже есть в списке")
            return 0



        json_now_post(message_to_save_add, one_post, str(message.from_user.id))    # модуль, добавдяющий в json нового автора (в качестве аргумента передаётся URL и первый пост автора)
...
def review1(message):                        # сей конструкцией мы получаем текст из телеграмма, в нашем случае URL на автора
    ...
    if (message_to_save_remove == "https://joyreactor.cc/tag/"):
    ...
    elif (message_to_save_remove.find("reactor.cc/tag/") != -1):        # ищем подстроку в строке и возвращаем индекс первого вхождения. для нас достаточно определить что эта подстрока впринципе есть 
        ...
        json_remove_avtor(message_to_save_remove, str(message.from_user.id))     # модуль, удаляющий в json автора (в качестве аргумента передаётся URL)
...
def test(message):
    buf = bot.reply_to(message, 'в ответ скинь мне ссылку на пост с видео')
    bot.register_next_step_handler(buf, pull2)

def pull2(message):                        # сей конструкцией мы получаем текст из телеграмма, в нашем случае URL на автора
